...
<div @include('crud::inc.field_wrapper_attributes') >
    ...
    <select type="text"
            name="{{ $field['name'] }}[]"
            multiple="multiple"
            @include('crud::inc.field_attributes', ['default_class' =>  'form-control select2allownew'])>
    @if (isset($field['model']))
        @foreach ($field['model']::all() as $connected_entity_entry)
            <option value="{{ $connected_entity_entry->getKey() }}"
                    ...
                    selected
                    @endif
            >{{ $connected_entity_entry->id }} - {{ $connected_entity_entry->{$field['attribute']} }}</option>
        @endforeach
    @endif
    </select>
...
@if ($crud->checkIfFieldIsFirstOfItsType($field, $fields))
    ...
    <script>
            ...
            $('.select2allownew').each(function (i, obj) {
                if (!$(obj).data("select2"))
                {
                    $(obj).select2({
                        tags: true
