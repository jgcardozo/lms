...
@section('content')
    ...
    <main>
        ...
        <div class="grid grid--full course-single"
            @if ($module->featured_image) style="background-image: url({{ $module->featured_image_url }});" @endif>
                ...
                <div class="module-lessons__list">
                    @foreach ($module->lessons as $key => $lesson)
                        <div id="lesson-{{ $lesson->id }}"
                            class="module-lessons__item grid--flex flex--space-between lesson {{ $key % 3 == 0 ? 'lesson--first' : '' }}">
                            <div class="lessons-list__content grid--flex flex--space-between flex--align-center">
                                ...
                                <div class="lessons-list__content--left">
                                    ...
                                    @else
                                        <h2 class="lessons-list__item--title"><a
                                                href="{{ route('single.lesson', $lesson->slug) }}">{{ $lesson->title }}</a>
                                        </h2>
                                    @endif
                                ...
                                <div class="lessons-list__content--right">
                                    @if ($lesson->is_completed && $lesson->isCompleteVideoFeatureOn())
                                        <div class="course-progress course-progress--completed course-progress__lesson">
                                            Completed <span
                                                class="course-progress__bar course-progress__bar--completed"></span></div>
                                    @elseif($lesson->is_locked)
                                        @if ($lesson->is_date_locked)
                                            <div class="course-progress course-progress__lesson"
                                                ...
                                                <span> Unlocks {{ Auth::user()->UnlockDate($lesson) }}</span> <span
                                                    class="course-progress__bar course-progress__bar--locked"></span>
                                            </div>
                                        @else
                                            <div class="course-progress course-progress__lesson">
                                    ...
                                    @else
                                        @if ($lesson->isCompleteVideoFeatureOn())
                                            <div class="course-progress course-progress__lesson"><span
                                                    class="course-progress__bar course-progress__bar--active"
                                                    data-percentage="{!! $lesson->getProgressPercentage() / 100 !!}"></span></div>
                                        @endif
                                    @endif
                                </div>
                            </div>
