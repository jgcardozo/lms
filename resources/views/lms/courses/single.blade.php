    ...
    <main>
                    ...
                    <div class="course-reminder__block-wrap grid--flex flex--space-between">
                        @if ($starterSeen === false)
                            <div class="course-reminder__content">
                                <h2 class="course-reminder__title">Getting Started: Begin by Watching these Intro Videos
                                    First</h2>
                                <p>How to find your way around, access important resources, and win great prizes!</p>
                            </div>

                            <div class="grid--flex flex--align-center">
                                <a href="{{ route('single.course.starter', $course->slug) }}"
                                    class="course-reminder__link {{ !empty($popupBefore) ? 'js-open-surveyPopup' : '' }}">Watch
                                    videos</a>
                            </div>
                        @elseif(!empty($nextSession) && $nextSession !== true)
                            ...
                            if ($t) {
                                ?>
                            <div class="grid--flex flex--align-center">
                                <a href="{{ route('single.lesson', $nextSession->lesson->slug) }}"
                                    class="course-reminder__link">Resume Lesson</a>
                            </div>
                            <?php
                            } else {
                                ?>
        ...
        <div class="grid grid--w950">
            <div class="course-modules">
                ...
                <div
                    class="grid--flex course-modules__list flex--row {{ $course->modules_not_hidden() === 4 ? 'four' : '' }} ">
                    @foreach ($course->modules as $key => $module)
                        @if ($module->locked_by_tag)
                            <div id="module-{{ $module->id }}"
                                class="module grid--flex {{ $key % 3 == 0 ? 'module--first' : '' }}"
                                @if ($module->is_locked && $module->module_status === 'hidden') style="display:none;" @endif>
                                <div class="module__component grid--flex flex--column">
                                    ...
                                    <div class="module__content">
                                        ...
                                        @else
                                            <a href="{{ route('single.module', $module->slug) }}" class="module__link">Go
                                                To Module</a>
                                        @endif
                                    </div>
                                </div>
